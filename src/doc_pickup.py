...
# --- Pickup request Create, Update, Get, and Delete examples (POST, PUT, GET, DELETE)
...
def pickup_CU(
    ...
    if verify_schema:
        ...
        schema, _ = get_schema(schema_override or f"{scac}.pickup.{method}")
    ...
    url = pickup_url.format(scac=scac)
    nparams = dict(params)
...
def pickup_get(
    ...
    **params: dict,
    ...
    if check_supported:
        supp = supported(scac, True)
        schema = f"{scac}.pickup.get".lower()
    ...
    with urllib.request.Request(url) as resp:
        text = resp.read()
        if resp.headers.get("content-type") == "application/json":
...
def pickup_delete(
    ...
    **params: dict,
    ...
    if check_supported:
        supp = supported(scac, True)
        schema = f"{scac}.pickup.delete".lower()
        if schema not in supp:
            raise ValueError(f"{schema} not supported")
    ...
    with urllib.request.Request(url, method="DELETE") as resp:
        text = resp.read()
        if resp.headers.get("content-type") == "application/json":
            return json.loads(text)
        # some upstreams return XML if raw or debug_upstream, so return the text unchagned
